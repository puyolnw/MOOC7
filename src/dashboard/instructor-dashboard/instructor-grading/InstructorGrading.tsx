...
const InstructorGrading: React.FC<InstructorGradingProps> = ({
    ...
    onClose,
    onGraded,
    onOpenGrading, // เพิ่ม prop นี้ใน destructuring
}) => {
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);
    const [questions, setQuestions] = useState<Question[]>([]);
    const [attemptAnswers, setAttemptAnswers] = useState<Answer[]>([]);
    const [attempts, setAttempts] = useState<Attempt[]>([]); // เพิ่ม state สำหรับรายการ attempts
    ...
    const loadData = async () => {
        try {
            ...
            if (isPopup && selectedAttemptId) {
                ...
                console.log("Fetching all attempts from:", `${apiURL}/api/special-quiz/attempts/all`);
                const response = await axios.get(
                ...
                console.log("Attempts response:", response.data);
    ...
    const handleSaveGrading = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(
                    response.data.message || "ไม่สามารถบันทึกการให้คะแนนได้"
                );
            }
        } catch (error) {
    ...
    if (!isPopup) {
        // แสดงรายการ attempts ในโหมด non-popup
        return (
            <div className="grading-container">
                <div className="grading-content">
                    <h4 className="mb-4">รายการการทำแบบทดสอบ</h4>
                    {attempts.length === 0 ? (
                        <div className="alert alert-info">
                            ไม่พบการทำแบบทดสอบที่ต้องตรวจ
                        </div>
                    ) : (
                                    ...
                                    <div className="card-body d-flex justify-content-between align-items-center">
                                        <div>
                                            ...
                                            <p className="card-text">
                                                <strong>อีเมล:</strong> {attempt.email}
                                            </p>
                                            <p className="card-text">
