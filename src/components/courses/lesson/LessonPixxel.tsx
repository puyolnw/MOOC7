      ...
      if (response.data.success) {
        ...
        if (response.data.lesson.progress) {
          ...
          if (sectionIndex !== -1) {
            ...
            if (itemIndex !== -1) {
              ...
              updatedLessonData[sectionIndex].items[itemIndex] = {
                ...updatedLessonData[sectionIndex].items[itemIndex],
                completed: response.data.lesson.progress.completed || updatedLessonData[sectionIndex].items[itemIndex].completed,
                duration: response.data.lesson.progress.completed ? "100%" : `${response.data.lesson.progress.progress || 0}%`
              };
              ...
              if (response.data.lesson.progress.completed) {
                ...
                if (hasQuiz && itemIndex + 1 < updatedLessonData[sectionIndex].items.length && 
          ...
          setLessonProgressMap(prev => ({
            ...prev,
            [String(lessonId)]: response.data.lesson.progress
          }));
        }
      }
    } catch (error) {
      ...
      if (response.data.progress) {
        ...
        if (sectionIndex !== -1) {
          ...
          if (itemIndex !== -1) {
            updatedLessonData[sectionIndex].items[itemIndex] = {
              ...updatedLessonData[sectionIndex].items[itemIndex],
              completed: response.data.progress.completed || updatedLessonData[sectionIndex].items[itemIndex].completed,
              duration: response.data.progress.completed ? "100%" : `${response.data.progress.progress || 0}%`
            };
            ...
            if (sectionId === 1 && response.data.progress.completed) {
              ...
              if (firstLessonSectionIndex !== -1 && updatedLessonData[firstLessonSectionIndex].items.length > 0) {
                updatedLessonData[firstLessonSectionIndex].items[0] = {
                  ...
                  lock: false
                      ...
                      setLessonProgressMap(prev => ({
                        ...prev,
                        [`quiz_${quizId}`]: response.data.progress
                      }));
              ...
              // ฟังก์ชันสำหรับตรวจสอบว่าบทเรียนมีแบบทดสอบท้ายบทหรือไม่
              ...
              export const findNextLessonAfterQuiz = (lessonData: SectionData[], sectionId: number, itemId: number) => {
                const section = lessonData.find(s => s.id === sectionId);
                if (!section) return null;
