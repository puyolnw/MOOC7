...
const LessonQuiz = ({
    ...
    const [selectedSingleAnswers, setSelectedSingleAnswers] = useState<
        number[]
    >([]);
    ...
    const fetchQuizStatus = async () => {
        try {
            ...
            if (response.data.success) {
                console.log("fetchQuizStatus", response.data)
    ...
    const fetchPassedQuizResults = async () => {
        try {
            ...
            if (response.data.success) {
                console.log("fetchPassedQuizResults", fetchPassedQuizResults)
        ...
        const fetchQuizData = async () => {
            ...
            try {
                ...
                if (quizData && quizData.length > 0) {
                    ...
                    const hasFillInBlank = formattedQuestions.some(
                        (q) => q.type === "FB"
                    );
                ...
                if (response.data.success && response.data.quiz) {
                    ...
                    const hasFillInBlank = response.data.quiz.questions.some(
                        (q: any) => q.type === "FB"
                    );
                    setIsSpecialQuiz(hasFillInBlank);
                    ...
                    if (latestAttempt) {
                        ...
                        if (
                            ...
                            setUploadedAttachments(
                                latestAttempt.answers.flatMap(
                                    (ans: Answer) => ans.attachments || []
                                )
                            );
                        }
                    }
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            const newFiles = Array.from(e.target.files).map((file) => ({
                ...
                question_id: questions[currentQuestion].question_id, // ดึง question_id จากคำถามปัจจุบัน
                file,
            }));
    ...
    const submitQuizAnswers = async () => {
        try {
                    ...
                    switch (question.type) {
                        ...
                        case "MC":
                            if (selectedMultipleAnswers[index]?.length > 0) {
                                ...
                                answer.choice_ids = selectedMultipleAnswers[
                                    index
                                ].map(
            ...
            console.log("grouped_files_question_ids:", grouped_files_question_ids);
            ...
            throw new Error(
                "การส่งแบบทดสอบล้มเหลว: " +
                    (response.data.message || "ไม่ทราบสาเหตุ")
            );
        } catch (error) {
                    ...
                    switch (question.type) {
                        ...
                        case "TF":
                            if (
                                ...
                                question.choices[selectedSingleAnswers[i]]
                                    ?.is_correct
                            ) {
    ...
    const isCurrentQuestionAnswered = () => {
        ...
        switch (question.type) {
            ...
            case "FB":
                ...
                const hasFiles =
                    files.filter((f) => f.questionIndex === currentQuestion)
                        .length > 0;
                ...
                <div className="result-container">
                    <div className="awaiting-review card shadow-sm p-4 text-center">
                        ...
                        <div className="message-box bg-light p-3 mb-4 rounded">
                            <p className="mb-2">
                                แบบทดสอบนี้เป็นแบบทดสอบที่ต้องรอการตรวจจากอาจารย์
                            </p>
                            <p className="mb-0">
    ...
    if (showResult) {
        ...
        return (
            <div className="quiz-container">
                <div className="result-container">
                    <div
                        className={`result card shadow-sm p-4 ${
                            isPassed
                    ...
                    >
                        ...
                        <h2 className="mb-4 fw-bold">
                            {isPassed
                                ? "ยินดีด้วย! คุณผ่านแบบทดสอบนี้"
                                : "คุณไม่ผ่านแบบทดสอบนี้"}
                        </h2>
                                                                            ...
                                                                            (
                                                                                ...
                                                                                answer
                                                                                    ...
                                                                                    <div
                                                                                        ...
                                                                                        className="answer-attachments mb-3"
                                                                                    >
                        ...
                        <div className="d-grid gap-2 col-md-6 mx-auto">
                            <button
                                className={`btn btn-lg ${
                                    isPassed ? "btn-success" : "btn-primary"
                                }`}
            ...
            <div className="question-container">
                <div className="question">
                    ...
                    <p className="question-type">
                        {questions[currentQuestion]?.type === "SC" &&
                            "(เลือกคำตอบเดียว)"}
                            ...
                            {questions[currentQuestion]?.choices.map(
                                (choice, index) => (
                                    <div
                                        ...
                                        onClick={() =>
                                            handleSingleAnswerSelect(index)
                                        }
                                    >
                            ...
                            {questions[currentQuestion]?.choices.map(
                                (choice, index) => (
                                    <div
                                        ...
                                        onClick={() =>
                                            handleMultipleAnswerSelect(index)
                                        }
                                    >
                                        <div className="option-marker">
                                            {selectedMultipleAnswers[
                    ...
                    {questions[currentQuestion]?.type === "FB" && (
                        <div className="text-answer">
                            ...
                            <div className="file-upload-section mt-3">
                                ...
                                {/* Show uploaded files */}
                                {(files.filter(
                                    (f) => f.questionIndex === currentQuestion
                                ).length > 0 ||
                                ...
                                <p className="text-muted small mt-2">
                                    ...
                                    สามารถอัปโหลดไฟล์ได้สูงสุด 10 ไฟล์
                                    ขนาดไม่เกิน 50MB ต่อไฟล์ (รองรับ .pdf, .doc,
                                    .docx, .xls, .xlsx)
                                </p>
                            </div>
                        </div>
